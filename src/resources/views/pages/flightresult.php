<div style="overflow-x:auto;">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="2" style="text-align:center;font-family:'Trebuchet MS', Arial, Helvetica, sans-serif;">
        <tr>
        ...
        <tr>
            <td valign="middle" style="border-top:1px solid #ccc;">
                <p>{{\Carbon\Carbon::parse($journey['departureTime'])->format('H:i')}}<br /> {{$journey['origin']}}</p>
            </td>
            ...
            <td valign="middle" style="border-top:1px solid #ccc;border-right:1px solid #ccc;">
                {{\Carbon\Carbon::parse($journey[0]['arrivalTime'])->format('H:i')}}<br />{{$journey['destination']}}
            </td>
            <td valign="middle" style="border-top:1px solid #ccc;">
                {{\Carbon\Carbon::parse($returnJourney['departureTime'])->format('H:i')}}<br />{{$returnJourney['origin']}}
            </td>
            <td valign="middle" style="border-top:1px solid #ccc;">
                <img src="{{URL::asset('assets/img/airlines/inbound.jpg')}}" alt="inbound flight" />
            </td>
            <td valign="middle">
                {{\Carbon\Carbon::parse($returnJourney['arrivalTime'])->format('H:i')}}<br />{{$returnJourney['destination']}}
            </td>
        </tr>
        <tr>
            <td height="30" colspan="3" valign="top" style="border-right:1px solid #ccc;">
                ...
                <div class="row flightsindetail hideflightdetails">
                    <?php if ( is_array($journey["flights"]) && !empty($journey["flights"])) {
            ...
            <?php if ($isReturn === true) { ?>
                <td colspan="3" valign="top">
                    ...
                    <div class="row flightsindetail hideflightdetails">
                        <?php if (is_array($returnJourney["flights"]) && !empty($returnJourney["flights"])) {
        ...
        <tr>
            <td colspan="6" valign="middle" style="border-top:1px solid #ccc;">
                <div class="grad_yellow">
                    <h2><?php echo $currencySymbol .
                        "" .
                        round($iti["pricing"]["totalAmount"]); ?></h2>
                    <form name="frm_bookNow" action="{{ url('/selectedflight') }}" method="POST">
                        <input type="hidden" name="itivalues" value='<?php echo json_encode(
                            $iti
                        ); ?>'/>
                        <input type="hidden" name="searchparams" value='<?php echo json_encode(
                            $params
                        ); ?>'/>
